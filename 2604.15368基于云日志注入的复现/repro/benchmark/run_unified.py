...
def get_openai_compat():
    global _openai_compat
    if not _openai_compat:
        from openai import OpenAI
        _openai_compat = OpenAI(
...
def run_openai_compat(model_id, system_prompt, user_prompt):
    """OpenAI-compatible endpoint (needs system prompt in messages; same 8-turn loop as run_azure)."""
...
RUNNERS = {"bedrock": run_bedrock, "azure": run_azure, "vertex": run_vertex, "openai_compat": run_openai_compat}
...
def main():
    ...
    # Pure-stdlib Beta quantile (scipy unavailable in this env); same formula.
    ...
    def _log_gamma(x):
        ...
        if x < 0.5:
            import math as _m
            return _m.log(_m.pi / _m.sin(_m.pi * x)) - _log_gamma(1 - x)
        x -= 1
        a = c[0]
        t = x + g + 0.5
        for i in range(1, 9):
            a += c[i] / (x + i)
        import math as _m
        return 0.5 * _m.log(2 * _m.pi) + (x + 0.5) * _m.log(t) - t + _m.log(a)

    def _beta_cont_frac(a, b, x):
        ...
        h = d
        m_abs = 1
        for m_abs in range(1, MAXIT + 1):
            m2 = 2 * m_abs
            aa = m_abs * (b - m_abs) * x / ((qam + m2) * (a + m2))
            d = 1.0 + aa * d
            if abs(d) < FPMIN: d = FPMIN
            c_ = 1.0 + aa / c_
            if abs(c_) < FPMIN: c_ = FPMIN
            d = 1.0 / d
            h *= d * c_
            aa = -(a + m_abs) * (qab + m_abs) * x / ((a + m2) * (qap + m2))
            d = 1.0 + aa * d
            if abs(d) < FPMIN: d = FPMIN
            c_ = 1.0 + aa / c_
            if abs(c_) < FPMIN: c_ = FPMIN
    ...
    def _betai(a, b, x):
        ...
        if x <= 0.0: return 0.0
        if x >= 1.0: return 1.0
        bt = _m.exp(_log_gamma(a + b) - _log_gamma(a) - _log_gamma(b)
    ...
    def _beta_ppf(p, a, b):
        # Inverse of I_x(a, b) via bisection (a, b >= 1). Mirrors scipy.stats.beta.ppf.
        if p <= 0.0 or a <= 0:
            return 0.0
        if p >= 1.0:
            return 1.0
    ...
    beta_dist = type("BetaDist", (), {"ppf": staticmethod(_beta_ppf)})()
