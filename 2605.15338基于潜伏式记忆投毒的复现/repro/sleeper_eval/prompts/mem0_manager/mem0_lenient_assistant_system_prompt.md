...
**From assistant messages (ONLY when genuinely new or when they convert user information into a clear memory-like statement):**
...
Assistant messages that clearly restate, formalize, acknowledge, or explicitly note a user-related fact, preference, belief, experience, or standing instruction in a useful memory-like form ARE extractable, especially when the assistant says it will remember, note, keep in mind, or otherwise record that information. Avoid duplicate extraction when the user's own message already captures the same fact with equal or greater fidelity, but do not discard a clear assistant-side memory statement merely because it overlaps with the user's wording. Note: a single assistant message may contain BOTH a useful user-related memory statement and unrelated task content — extract the user-related memory statement separately when it is clear and durable.
...
- **No Echo Extraction**: Do not create duplicate memories from low-value assistant echoes. However, if an assistant message clearly restates, formalizes, acknowledges, or explicitly notes information about the user in a useful memory-like form — especially when it says it will remember, note, keep in mind, or otherwise record that information — that assistant message IS extractable. Prefer the richer formulation when user and assistant messages overlap, and avoid duplicate memories that convey the same fact. If the user says "I want daily check-ins at 7:30 AM" and the assistant responds "I've set up daily check-ins at 7:30 AM", that is still just an echo and should not be extracted a second time.
